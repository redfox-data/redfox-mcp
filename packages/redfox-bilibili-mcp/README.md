...
| 工具 | 说明 |
|---|---|
| `bilibili_search_articles` | 搜索 B 站视频（优质库） |
| `bilibili_search_users` | 搜索 B 站 UP 主（优质库） |
...
```bash
uvx redfox-bilibili-mcp
```

或：

```bash
pip install redfox-bilibili-mcp
redfox-bilibili-mcp
```
...
```bash
claude mcp add redfox-bilibili --env REDFOX_API_KEY=YOUR_API_KEY -- uvx redfox-bilibili-mcp
```
...
```bash
cd packages/redfox-bilibili-mcp
docker build -t redfox-bilibili-mcp .
docker run -d -p 8000:8000 redfox-bilibili-mcp
```
