...
RedFoxHub X(Twitter) 数据 MCP Server — 将 X(Twitter) 数据 API 封装为 4 个 MCP 工具，适用于 dsh、Claude Code、Cursor 等任意 MCP 客户端。

## 工具（4）

| 工具 | 说明 |
|---|---|
| `twitter_search_tweets` | X(Twitter) 关键词搜索推文 |
| `twitter_get_tweet` | 获取 X(Twitter) 单条推文详情 |
| `twitter_get_user` | 获取 X(Twitter) 用户信息 |
| `twitter_get_comments` | 获取 X(Twitter) 推文评论/回复 |
...
```bash
uvx redfox-twitter-mcp
```

或：

```bash
pip install redfox-twitter-mcp
redfox-twitter-mcp
```
...
```bash
claude mcp add redfox-twitter --env REDFOX_API_KEY=YOUR_API_KEY -- uvx redfox-twitter-mcp
```
...
```bash
cd packages/redfox-twitter-mcp
docker build -t redfox-twitter-mcp .
docker run -d -p 8000:8000 redfox-twitter-mcp
```
