...
RedFoxHub YouTube 数据 MCP Server — 将 YouTube 数据 API 封装为 4 个 MCP 工具，适用于 dsh、Claude Code、Cursor 等任意 MCP 客户端。

## 工具（4）

| 工具 | 说明 |
|---|---|
| `youtube_search_videos` | YouTube 关键词视频搜索 |
| `youtube_get_video` | 获取 YouTube 单个视频详情 |
| `youtube_get_comments` | 获取 YouTube 视频评论 |
| `youtube_get_transcript` | 提取 YouTube 视频字幕/文案 |
...
```bash
uvx redfox-youtube-mcp
```

或：

```bash
pip install redfox-youtube-mcp
redfox-youtube-mcp
```
...
```bash
claude mcp add redfox-youtube --env REDFOX_API_KEY=YOUR_API_KEY -- uvx redfox-youtube-mcp
```
...
```bash
cd packages/redfox-youtube-mcp
docker build -t redfox-youtube-mcp .
docker run -d -p 8000:8000 redfox-youtube-mcp
```
